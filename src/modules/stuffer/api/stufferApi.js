...
export const useDeleteThing = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['stf_things'] }),
  });
};

// Инкремент счётчика открытий — fire and forget, без инвалидации
...
export const useSaveRegister = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      // Инвалидируем список вещей (статус/локация изменились)
      qc.invalidateQueries({ queryKey: ['stf_things'] });
      qc.invalidateQueries({ queryKey: ['stf_thing', variables.thing_id] });
      // Инвалидируем регистр
