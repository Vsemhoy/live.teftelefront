  ...
  return (
    <Box style={{ position: 'relative', margin: '12px 0' }}>
      ...
      <SyntaxHighlighter style={oneLight} language={lang || 'text'} PreTag="div"
        showLineNumbers customStyle={{ fontSize: 13, margin: 0, borderRadius: 6, overflowX: 'auto' }}>
        {code}
      </SyntaxHighlighter>
    </Box>
  );
...
const readComponents = {
  h1: ({ children }) => <Text component="h1" size="xl" fw={700} mt="md" mb={6} style={{ lineHeight: 1.3 }}>{children}</Text>,
  h2: ({ children }) => <Text component="h2" size="lg" fw={600} mt="md" mb={4} style={{ lineHeight: 1.3 }}>{children}</Text>,
  h3: ({ children }) => <Text component="h3" size="md" fw={600} mt={8} mb={2} style={{ lineHeight: 1.3 }}>{children}</Text>,
  ...
  blockquote: ({ children }) => (
    <Box style={{ borderLeft: '3px solid var(--mantine-color-blue-3)', paddingLeft: 14, margin: '10px 0', background: 'var(--mantine-color-blue-0)', borderRadius: '0 4px 4px 0', padding: '8px 14px' }}>
      {children}
    </Box>
  ),
  ...
  table: ({ children }) => <Box style={{ overflowX: 'auto', marginBottom: 12 }}><table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>{children}</table></Box>,
  th: ({ children }) => <th style={{ border: '1px solid var(--mantine-color-gray-3)', padding: '6px 12px', background: 'var(--mantine-color-gray-1)', fontWeight: 600, textAlign: 'left' }}>{children}</th>,
  td: ({ children }) => <td style={{ border: '1px solid var(--mantine-color-gray-2)', padding: '5px 12px' }}>{children}</td>,
  hr: () => <Divider my="md" />,
};
