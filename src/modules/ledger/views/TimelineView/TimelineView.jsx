...
import { useTransactions, useAccounts, useMonthTotals, useMoveTransaction, useSaveTransaction, useDeleteTransaction, useToggleTransaction } from '../../api/ledgerApi';
import { formatMoney, flowKindColor, flowKindSign, calcDailyInterest } from '../../utils/ledgerUtils';
...
const DraggableCard = ({ transaction, onDoubleClick }) => {
  ...
  const { openEditor, openDuplicator } = useLedgerStore();
  ...
  return (
    <div
      ref={setNodeRef}
      className="led-transaction-card"
      style={{
...
const DroppableSlot = ({ dateStr, accountId, children, onAdd, isDisabled }) => {
  ...
  return (
    <div
      ...
      onDoubleClick={(e) => {
        if (e.target.closest('.led-transaction-card')) return;
...
const SlotBalance = ({ balance, mode, currency, totals, dailyInterest }) => {
  ...
  if (balance === null) return (
    <div className="led-slot-balance">
  ...
  if (mode === 'extended' && totals) {
    return (
      <div className="led-slot-balance extended">
...
const MonthTotalsRow = ({ monthKey, activeAccounts, accounts, transactions, balanceByAccount, closingDateStr, label, variant = 'closing' }) => {
  ...
  return (
    <div className={`led-month-totals-row ${isOpening ? 'led-month-totals-opening' : ''}`}>
      <div className="led-month-totals-label">
      ...
      {activeAccounts.map((accId) => {
        ...
        return (
          <div key={accId} className="led-month-totals-slot">
      ...
      <div className="led-month-totals-total">
...
const DayRow = ({ date, activeAccounts, accounts, txByAccountByDate, balanceByAccount, interestByAccount, balanceMode, onAdd, onCardDoubleClick, isToday, stripe }) => {
  ...
  return (
    <div className="led-day-row" id={isToday ? 'today_row' : undefined} style={{ background: rowBg }}>
      <div className={`led-date-label ${isToday ? 'today' : ''}`}>
        {isToday ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
            ...
            <div className="led-day-name">{dayName}</div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
            <div className="led-day-num" style={{
              color: isWeekend ? 'var(--mantine-color-red-4)' : 'var(--mantine-color-gray-7)',
            }}>{dayNum}</div>
            <div className="led-day-name" style={{
...
export const TimelineView = () => {
  ...
  // IntersectionObserver — следим за .led-month-header, пишем верхний видимый месяц
  useEffect(() => {
    const headers = document.querySelectorAll('.led-month-header[data-month]');
  ...
  const activeAccounts  = useLedgerStore((s) => s.activeAccounts);
  const balanceMode     = useLedgerStore((s) => s.balanceMode);
  const openEditor      = useLedgerStore((s) => s.openEditor);
  const openReader      = useLedgerStore((s) => s.openReader);
  const duplicatorOpen  = useLedgerStore((s) => s.duplicatorOpen);
  const categoryFilter  = useLedgerStore((s) => s.categoryFilter);
  ...
  return (
    ...
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div
        className="content-scroll led-timeline"
        style={{
          paddingBottom: 80,
      ...
      >
        ...
        {/* Шапка */}
        <div className="led-timeline-header">
          <div className="led-date-label" style={{ visibility: 'hidden' }}>00</div>
          {activeAccounts.map((accId) => {
            const acc = accounts.find((a) => a.id === accId);
            return (
              <div key={accId} className="led-account-slot led-account-header">
        ...
        {monthKeys.map((monthKey, monthIndex) => {
          ...
          return (
            <div key={monthKey}>
              ...
              <div className="led-month-header" data-month={monthKey}>
